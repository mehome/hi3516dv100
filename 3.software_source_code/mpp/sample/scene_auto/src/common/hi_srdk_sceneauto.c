...
HI_S32 SceneAuto_SetAutoFSWDR()
{
    ...
    //printf("%s: u8FSWDRMode = %u\n", __FUNCTION__, stAdptAEAttr.u8FSWDRMode);
...
HI_S32 SceneAuto_SetAERealtedBit(HI_S32 s32IspDev, HI_S32 s32AEBitrateLevel)
{
    ...
    if ((SRDK_SCENEAUTO_SPECIAL_SCENE_MANUAL_NORMAL_WDR != g_eSpecialScene) && (SRDK_SCENEAUTO_SPECIAL_SCENE_MANUAL_LONG_FRAME != g_eSpecialScene) && \
        (SRDK_SCENEAUTO_SPECIAL_SCENE_AUTO_FSWDR != g_eSpecialScene) && (SRDK_SCENEAUTO_SPECIAL_SCENE_IR != g_eSpecialScene) && \
        (SRDK_SCENEAUTO_SPECIAL_SCENE_BLC != g_eSpecialScene) && (SRDK_SCENEAUTO_SPECIAL_SCENE_HLC != g_eSpecialScene))
    {
        s32Ret = CommSceneautoGetAEAttr(s32IspDev, &stAdptAEAttr);
        if (HI_SUCCESS != s32Ret)
        {
...
HI_S32 Scenauto_SetAutoFSWDRAERealtedExp(HI_S32 s32IspDev, HI_S32 s32AutoFSWDRAECurPos)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ADPT_SCENEAUTO_AEATTR_S stAdptAEAttr;
    ...
    if(1 == stAdptAEAttr.u8FSWDRMode)
    {
        stAdptAEAttr.u8ExpCompensation = g_stINIPara.stAutoFSWDR.pu8ExpCompensation[s32AutoFSWDRAECurPos];
        stAdptAEAttr.u8MaxHistOffset = g_stINIPara.stAutoFSWDR.pu8MaxHistOffset[s32AutoFSWDRAECurPos];
...
HI_S32 SceneAuto_SetPreviousPara()
{
    ...
    stAdptDRCAttr.u32AutoStrength = g_stPreviousPara.stDRCAttr.s32AutoStrength;
    ...
    stAdptAEAttr.u8FSWDRMode = g_stPreviousPara.stAEAttr.u8FSWDRMode;
    stAdptAEAttr.bWDRQuick = g_stPreviousPara.stAEAttr.bWDRQuick;
    for (i = 0; i < AE_WEIGHT_ROW; i++)
    {
        for (j = 0; j < AE_WEIGHT_COLUMN; j++)
        {
...
HI_S32 Sceneauto_GetPreviousPara()
{
    ...
    g_stPreviousPara.stDRCAttr.s32AutoStrength = stAdptDRCAttr.u32AutoStrength;
    ...
    g_stPreviousPara.stAEAttr.bWDRQuick = stAdptAEAttr.bWDRQuick;
    g_stPreviousPara.stAEAttr.u8FSWDRMode = stAdptAEAttr.u8FSWDRMode;
    for (i = 0; i < AE_WEIGHT_ROW; i++)
    {
        for (j = 0; j < AE_WEIGHT_COLUMN; j++)
        {
...
HI_S32 Sceneauto_LoadINIPara()
{
	...
	/**************common:u16RatioBias**************/
    s32Temp = 0;
    s32Temp = iniparser_getint(g_Sceneautodictionary, "common:u16RatioBias", HI_FAILURE);
    if (HI_FAILURE == s32Temp)
    {
        printf("common:u16RatioBias failed\n");
        return HI_FAILURE;
    }
    g_stINIPara.u16RatioBias = s32Temp;
    ...
    //AE: aeWDRQuick
    s32Temp = 0;
    s32Temp = iniparser_getint(g_Sceneautodictionary, "AE:aeWDRQuick", HI_FAILURE);
    if (HI_FAILURE == s32Temp)
    {
        printf("AE:aeWDRQuick failed\n");
        return HI_FAILURE;
    }
    g_stINIPara.stIniAE.bWDRQuick = (HI_BOOL)s32Temp;
    //AE:aeBitrateCount
    s32Temp = 0;
    ...
    g_stINIPara.stAutoFSWDR.u32ISOUpTh = (HI_BOOL)s32Temp;
    ...
    g_stINIPara.stAutoFSWDR.u32ISODnTh = (HI_BOOL)s32Temp;
    ...
    g_stINIPara.stAutoFSWDR.u32RefExpRatioTh = (HI_BOOL)s32Temp;
    ...
    g_stINIPara.stAutoFSWDR.s32ExpCount = s32Temp;
    g_stINIPara.stAutoFSWDR.pu32ExpThreshLtoH = (HI_U32*)malloc((g_stINIPara.stAutoFSWDR.s32ExpCount) * sizeof(HI_U32));
    CHECK_NULL_PTR(g_stINIPara.stAutoFSWDR.pu32ExpThreshLtoH);
    g_stINIPara.stAutoFSWDR.pu32ExpThreshHtoL = (HI_U32*)malloc((g_stINIPara.stAutoFSWDR.s32ExpCount) * sizeof(HI_U32));
    CHECK_NULL_PTR(g_stINIPara.stAutoFSWDR.pu32ExpThreshHtoL);
    g_stINIPara.stAutoFSWDR.pu8ExpCompensation = (HI_U8*)malloc((g_stINIPara.stAutoFSWDR.s32ExpCount) * sizeof(HI_U8));
    CHECK_NULL_PTR(g_stINIPara.stAutoFSWDR.pu8ExpCompensation);
    g_stINIPara.stAutoFSWDR.pu8MaxHistOffset = (HI_U8*)malloc((g_stINIPara.stAutoFSWDR.s32ExpCount) * sizeof(HI_U8));
    CHECK_NULL_PTR(g_stINIPara.stAutoFSWDR.pu8MaxHistOffset);
    ...
    s32Temp = Weight(pszTempStr);
    for (i = 0; i < g_stINIPara.stAutoFSWDR.s32ExpCount; i++)
    {
        g_stINIPara.stAutoFSWDR.pu32ExpThreshLtoH[i] = MAEWeight[i];
    }
    ...
    for (i = 0; i < g_stINIPara.stAutoFSWDR.s32ExpCount; i++)
    {
        g_stINIPara.stAutoFSWDR.pu32ExpThreshHtoL[i] = MAEWeight[i];
    }
    ...
    for (i = 0; i < g_stINIPara.stAutoFSWDR.s32ExpCount; i++)
    {
        g_stINIPara.stAutoFSWDR.pu8ExpCompensation[i] = MAEWeight[i];
    }
    ...
    for (i = 0; i < g_stINIPara.stAutoFSWDR.s32ExpCount; i++)
    {
        g_stINIPara.stAutoFSWDR.pu8MaxHistOffset[i] = MAEWeight[i];
    }
    ...
    g_stINIPara.stAutoFSWDR.u32DRCStrength = (HI_BOOL)s32Temp;
    ...
    if (HI_FAILURE == s32Temp)
    {
    	printf("stAutoFSWDR:3DnrIsoCount failed\n");
        return HI_FAILURE;
    }    
    g_stINIPara.stAutoFSWDR.s323DnrIsoCount = s32Temp;
    g_stINIPara.stAutoFSWDR.pu323DnrIsoThresh = (HI_U32 *)malloc((g_stINIPara.stAutoFSWDR.s323DnrIsoCount) * sizeof(HI_U32));
    CHECK_NULL_PTR(g_stINIPara.stAutoFSWDR.pu323DnrIsoThresh);
    g_stINIPara.stAutoFSWDR.pst3dnrParam = (SCENEAUTO_INIPARAM_3DNRCFG_S *)malloc((g_stINIPara.stAutoFSWDR.s323DnrIsoCount) * sizeof(SCENEAUTO_INIPARAM_3DNRCFG_S));
    CHECK_NULL_PTR(g_stINIPara.stAutoFSWDR.pst3dnrParam);

    //stAutoFSWDR:3DnrIsoThresh
    pszTempStr = iniparser_getstr(g_Sceneautodictionary, "FSWDR:3DnrIsoThresh");
    if (NULL == pszTempStr)
    {
        printf("FSWDR:3DnrIsoThresh error\n");
        return HI_FAILURE;
    }
    s32Temp = Weight(pszTempStr);
    for (i = 0; i < g_stINIPara.stAutoFSWDR.s323DnrIsoCount; i++)
    {
        g_stINIPara.stAutoFSWDR.pu323DnrIsoThresh[i] = MAEWeight[i];
    }
    for (i = 0; i < g_stINIPara.stAutoFSWDR.s323DnrIsoCount; i++)
    {
        //stAutoFSWDR:SBS
        snprintf(szTempStr, sizeof(szTempStr), "FSWDR:SBS_%d", i);
        pszTempStr = iniparser_getstr(g_Sceneautodictionary, szTempStr);
        if (NULL == pszTempStr)
        {
            printf("FSWDR:SBS_%d error\n", i);
            return HI_FAILURE;
        }
        s32Temp = Weight(pszTempStr);
        for (j = 0; j < 3; j++)
        {
            g_stINIPara.stAutoFSWDR.pst3dnrParam[i].u8SBS[j] = MAEWeight[j];
        }
        //stAutoFSWDR:PSFS
        snprintf(szTempStr, sizeof(szTempStr), "FSWDR:PSFS_%d", i);
        s32Temp = 0;
        s32Temp = iniparser_getint(g_Sceneautodictionary, szTempStr, HI_FAILURE);
        if (HI_FAILURE == s32Temp)
        {
            printf("FSWDR:PSFS_%d failed\n", i);
            return HI_FAILURE;
        }
        g_stINIPara.stAutoFSWDR.pst3dnrParam[i].u16PSFS = s32Temp;
        //stAutoFSWDR:SBT
        snprintf(szTempStr, sizeof(szTempStr), "FSWDR:SBT_%d", i);
        pszTempStr = iniparser_getstr(g_Sceneautodictionary, szTempStr);
        if (NULL == pszTempStr)
        {
            printf("FSWDR:SBT_%d error\n", i);
            return HI_FAILURE;
        }
        s32Temp = Weight(pszTempStr);
        for (j = 0; j < 3; j++)
        {
            g_stINIPara.stAutoFSWDR.pst3dnrParam[i].u8SBT[j] = MAEWeight[j];
        }
        //stAutoFSWDR:SDS
        snprintf(szTempStr, sizeof(szTempStr), "FSWDR:SDS_%d", i);
        pszTempStr = iniparser_getstr(g_Sceneautodictionary, szTempStr);
        if (NULL == pszTempStr)
        {
            printf("FSWDR:SDS_%d error\n", i);
            return HI_FAILURE;
        }
        s32Temp = Weight(pszTempStr);
        for (j = 0; j < 3; j++)
        {
            g_stINIPara.stAutoFSWDR.pst3dnrParam[i].u8SDS[j] = MAEWeight[j];
        }
        //stAutoFSWDR:SDT
        snprintf(szTempStr, sizeof(szTempStr), "FSWDR:SDT_%d", i);
        pszTempStr = iniparser_getstr(g_Sceneautodictionary, szTempStr);
        if (NULL == pszTempStr)
        {
            printf("FSWDR:SDT_%d error\n", i);
            return HI_FAILURE;
        }
        s32Temp = Weight(pszTempStr);
        for (j = 0; j < 3; j++)
        {
            g_stINIPara.stAutoFSWDR.pst3dnrParam[i].u8SDT[j] = MAEWeight[j];
        }
        //stAutoFSWDR:SBF
        snprintf(szTempStr, sizeof(szTempStr), "FSWDR:SBF_%d", i);
        pszTempStr = iniparser_getstr(g_Sceneautodictionary, szTempStr);
        if (NULL == pszTempStr)
        {
            printf("FSWDR:SBF_%d error\n", i);
            return HI_FAILURE;
        }
        s32Temp = Weight(pszTempStr);
        for (j = 0; j < 3; j++)
        {
            g_stINIPara.stAutoFSWDR.pst3dnrParam[i].u16SBF[j] = MAEWeight[j];
        }
        //stAutoFSWDR:SFC
        snprintf(szTempStr, sizeof(szTempStr), "FSWDR:SFC_%d", i);
        s32Temp = 0;
        s32Temp = iniparser_getint(g_Sceneautodictionary, szTempStr, HI_FAILURE);
        if (HI_FAILURE == s32Temp)
        {
            printf("FSWDR:SFC_%d failed\n", i);
            return HI_FAILURE;
        }
        g_stINIPara.stAutoFSWDR.pst3dnrParam[i].u8SFC = s32Temp;
        //stAutoFSWDR:SHP
        snprintf(szTempStr, sizeof(szTempStr), "FSWDR:SHP_%d", i);
        pszTempStr = iniparser_getstr(g_Sceneautodictionary, szTempStr);
        if (NULL == pszTempStr)
        {
            printf("FSWDR:SHP_%d error\n", i);
            return HI_FAILURE;
        }
        s32Temp = Weight(pszTempStr);
        for (j = 0; j < 3; j++)
        {
            g_stINIPara.stAutoFSWDR.pst3dnrParam[i].u8SHP[j] = MAEWeight[j];
        }
        //stAutoFSWDR:TFC
        snprintf(szTempStr, sizeof(szTempStr), "FSWDR:TFC_%d", i);
        s32Temp = 0;
        s32Temp = iniparser_getint(g_Sceneautodictionary, szTempStr, HI_FAILURE);
        if (HI_FAILURE == s32Temp)
        {
            printf("FSWDR:TFC_%d failed\n", i);
            return HI_FAILURE;
        }
        g_stINIPara.stAutoFSWDR.pst3dnrParam[i].u8TFC = s32Temp;
        //stAutoFSWDR:TFS
        snprintf(szTempStr, sizeof(szTempStr), "FSWDR:TFS_%d", i);
        pszTempStr = iniparser_getstr(g_Sceneautodictionary, szTempStr);
        if (NULL == pszTempStr)
        {
            printf("FSWDR:TFS_%d error\n", i);
            return HI_FAILURE;
        }
        s32Temp = Weight(pszTempStr);
        for (j = 0; j < 3; j++)
        {
            g_stINIPara.stAutoFSWDR.pst3dnrParam[i].u16TFS[j] = MAEWeight[j];
        }
        //stAutoFSWDR:TFR
        snprintf(szTempStr, sizeof(szTempStr), "FSWDR:TFR_%d", i);
        pszTempStr = iniparser_getstr(g_Sceneautodictionary, szTempStr);
        if (NULL == pszTempStr)
        {
            printf("FSWDR:TFR_%d error\n", i);
            return HI_FAILURE;
        }
        s32Temp = Weight(pszTempStr);
        for (j = 0; j < 3; j++)
        {
            g_stINIPara.stAutoFSWDR.pst3dnrParam[i].u16TFR[j] = MAEWeight[j];
        }
        //stAutoFSWDR:MDZ
        snprintf(szTempStr, sizeof(szTempStr), "FSWDR:MDZ_%d", i);
        pszTempStr = iniparser_getstr(g_Sceneautodictionary, szTempStr);
        if (NULL == pszTempStr)
        {
            printf("FSWDR:MDZ_%d error\n", i);
            return HI_FAILURE;
        }
        s32Temp = Weight(pszTempStr);
        for (j = 0; j < 3; j++)
        {
            g_stINIPara.stAutoFSWDR.pst3dnrParam[i].u16MDZ[j] = MAEWeight[j];
        }
        //stAutoFSWDR:POST
        snprintf(szTempStr, sizeof(szTempStr), "FSWDR:POST_%d", i);
        s32Temp = 0;
        s32Temp = iniparser_getint(g_Sceneautodictionary, szTempStr, HI_FAILURE);
        if (HI_FAILURE == s32Temp)
        {
            printf("FSWDR:POST_%d failed\n", i);
            return HI_FAILURE;
        }
        g_stINIPara.stAutoFSWDR.pst3dnrParam[i].u16POST = s32Temp;  
    } 
...
void* SceneAuto_NormalThread(void* pVoid)
{
    ...
    HI_U32 u32LightGain;
    HI_U32 u32WDRCount0 = 0;
    HI_U32 u32WDRCount1 = 0;
    ...
    while (HI_TRUE == g_bNormalThreadFlag)
    {
            ...
            (eSpecialScene != g_eSpecialScene))
        {
            ...
            else if (((SRDK_SCENEAUTO_SPECIAL_SCENE_AUTO_FSWDR == g_eSpecialScene) && (1 == stAdptAEAttr.u8FSWDRMode)) || \
                (SRDK_SCENEAUTO_SPECIAL_SCENE_MANUAL_LONG_FRAME == g_eSpecialScene))
            {
                for (s32IsoLevel = 0; s32IsoLevel < g_stINIPara.stAutoFSWDR.s323DnrIsoCount; s32IsoLevel++)
                {
                    if (u32Iso <= g_stINIPara.stAutoFSWDR.pu323DnrIsoThresh[s32IsoLevel])
                    {
                        break;
                    }
                }
                if (s32IsoLevel == g_stINIPara.stAutoFSWDR.s323DnrIsoCount)
                {
                    s32IsoLevel = g_stINIPara.stAutoFSWDR.s323DnrIsoCount - 1;
                }

                Interpolate(&stSceneauto3dnr, u32Iso, 
                        &g_stINIPara.stAutoFSWDR.pst3dnrParam[s32IsoLevel - 1], g_stINIPara.stAutoFSWDR.pu323DnrIsoThresh[s32IsoLevel - 1],
                        &g_stINIPara.stAutoFSWDR.pst3dnrParam[s32IsoLevel], g_stINIPara.stAutoFSWDR.pu323DnrIsoThresh[s32IsoLevel]);  
            }
            else
            {
            ...
            else if ((SRDK_SCENEAUTO_SPECIAL_SCENE_AUTO_FSWDR == g_eSpecialScene) || (SRDK_SCENEAUTO_SPECIAL_SCENE_MANUAL_LONG_FRAME == g_eSpecialScene) || \
                (SRDK_SCENEAUTO_SPECIAL_SCENE_MANUAL_NORMAL_WDR == g_eSpecialScene))
            {
                if(SRDK_SCENEAUTO_SPECIAL_SCENE_MANUAL_LONG_FRAME == g_eSpecialScene)
                {
                    stAdptAEAttr.u8FSWDRMode = 1;
                }
                else if(SRDK_SCENEAUTO_SPECIAL_SCENE_MANUAL_NORMAL_WDR == g_eSpecialScene)
                {
                    stAdptAEAttr.u8FSWDRMode = 0;
                }
                else
                {
                    if ((stAdptExposureInfo.u32ISO > g_stINIPara.stAutoFSWDR.u32ISOUpTh) && (stAdptExposureInfo.u32ISO > g_stINIPara.stAutoFSWDR.u32RefExpRatioTh))
                    {
                        u32WDRCount0++;
                    }
                    else
                    ...
                    if ((stAdptExposureInfo.u32ISO < g_stINIPara.stAutoFSWDR.u32ISODnTh))
                    {
                        u32WDRCount1++;
                    }
                    else
                ...
                if(1 == stAdptAEAttr.u8FSWDRMode)
                {
                    stAdptDrcAttr.u32AutoStrength = 0;
                }
                else
                {
                    stAdptDrcAttr.u32AutoStrength = 128;
                }
                s32Ret = CommSceneautoSetDRCAttr(s32IspDev, &stAdptDrcAttr);
                if (HI_SUCCESS != s32Ret)
                {
                ...
                //////// WDR ratio bias 
                ADPT_SCENEAUTO_WDREXPOSUREATTR_S stAdptWdrExposureAttr;
                HI_U16 u16RatioBias = 0x400;
                static HI_U16 u16RatioBiasCnt = 0;
                if (0 == stAdptAEAttr.u8FSWDRMode)
                {
                    if (u16RatioBiasCnt < 2)
                    {
                        u16RatioBiasCnt++;
                    }
                    else
                    {
                        u16RatioBias = g_stINIPara.u16RatioBias;
                    }
                }
                else    
                {
                ...
                //printf("Cnt %d   Bias %d\n", u16RatioBiasCnt, u16RatioBias);
            	s32Ret = CommSceneautoGetWdrExposureAttr(s32IspDev, &stAdptWdrExposureAttr);
            	if (HI_SUCCESS != s32Ret)
                {
                    printf("CommSceneautoGetWdrExposureAttr failed\n");
                }
                stAdptWdrExposureAttr.u16RatioBias = u16RatioBias;
                s32Ret = CommSceneautoSetWdrExposureAttr(s32IspDev, &stAdptWdrExposureAttr);
            	if (HI_SUCCESS != s32Ret)
                {
                    printf("CommSceneautoSetWdrExposureAttr failed\n");
                }
                //////// WDR ratio bias end
                                
                if (u32Exposure > u32LastExposure)
                {
                    for (i = s32AutoFSWDRAECurPos; i < g_stINIPara.stAutoFSWDR.s32ExpCount; i++)
                    {
                        if (u32Exposure > g_stINIPara.stAutoFSWDR.pu32ExpThreshLtoH[i])
                        {
                            s32AutoFSWDRAECurPos = i + 1;
                        }
                        else
                        {
                            break;
                    ...
                    if (s32AutoFSWDRAECurPos >= g_stINIPara.stAutoFSWDR.s32ExpCount)
                    {
                        s32AutoFSWDRAECurPos = g_stINIPara.stAutoFSWDR.s32ExpCount - 1;
                    }
                }
                else if (u32Exposure < u32LastExposure)
                {
                    for (i = s32AutoFSWDRAECurPos; i >= 0; i--)
                    {
                        if (u32Exposure < g_stINIPara.stAutoFSWDR.pu32ExpThreshHtoL[i])
                        {
                            s32AutoFSWDRAECurPos = i;
                        }
                        else
                        {
                            break;
                ...
                s32Ret = Scenauto_SetAutoFSWDRAERealtedExp(s32IspDev, s32AutoFSWDRAECurPos);
                if (HI_SUCCESS != s32Ret)
                {
                    printf("Scenauto_SetAERealtedExp failed\n");
                }
                s32Ret = SceneAuto_SetAutoFSWDR();
                if (HI_SUCCESS != s32Ret)
                {
...
HI_S32 HI_SRDK_SCENEAUTO_SetSpecialMode(const SRDK_SCENEAUTO_SEPCIAL_SCENE_E* peSpecialScene)
{
    ...
    if ((*peSpecialScene != SRDK_SCENEAUTO_SPECIAL_SCENE_BLC) &&
        ...
        (*peSpecialScene != SRDK_SCENEAUTO_SPECIAL_SCENE_AUTO_FSWDR) && 
        (*peSpecialScene != SRDK_SCENEAUTO_SPECIAL_SCENE_MANUAL_LONG_FRAME) && 
        (*peSpecialScene != SRDK_SCENEAUTO_SPECIAL_SCENE_MANUAL_NORMAL_WDR))
    {
    ...
    if (g_eSpecialScene != eSpecialScene)
    {
        ...
        switch (eSpecialScene)
        {
            ...
            case SRDK_SCENEAUTO_SPECIAL_SCENE_DRC:
                ...
                break;
            case SRDK_SCENEAUTO_SPECIAL_SCENE_AUTO_FSWDR:
            case SRDK_SCENEAUTO_SPECIAL_SCENE_MANUAL_LONG_FRAME:
            case SRDK_SCENEAUTO_SPECIAL_SCENE_MANUAL_NORMAL_WDR: 
                s32Ret = SceneAuto_SetAutoFSWDR();
                if (HI_SUCCESS != s32Ret)
                {
                    printf("SceneAuto_SetAutoFSWDR failed\n");
...
HI_S32 HI_SRDK_SCENEAUTO_Start()
{
    ...
    stAdptAEAttr.bWDRQuick = g_stINIPara.stIniAE.bWDRQuick;
    s32Ret = CommSceneautoSetAEAttr(s32IspDev, &stAdptAEAttr);
    if (HI_SUCCESS != s32Ret)
    {
        printf("CommSceneautoSetAEAttr failed\n");
    ...
    stAdptWdrExposureAttr.u16RatioBias = g_stINIPara.u16RatioBias;
    s32Ret = CommSceneautoSetWdrExposureAttr(s32IspDev, &stAdptWdrExposureAttr);
    if (HI_SUCCESS != s32Ret)
    {
        printf("CommSceneautoSetWdrExposureAttr failed\n");
