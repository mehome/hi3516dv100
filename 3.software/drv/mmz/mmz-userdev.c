...
static unsigned long usr_virt_to_phys(unsigned long virt)
{
	pgd_t *pgd;
	pud_t *pud;
	pmd_t *pmd;
	...
	if (virt & 0x3) {
		error("Invalid virtual address 0x%lx[not 4 bytes align]\n", virt);
		return 0;
	}

	if (virt >= CONFIG_PAGE_OFFSET) {
		error("invalid usr virtual address 0x%lx\n", virt);
		return 0;
	}
...
static int ioctl_mmb_virt2phys(struct file *file, unsigned int iocmd, struct mmb_info *pmi)
{
	...
	if (!hil_mmb_getby_phys_2(phys, &offset)) {
		//error("Not mmz alloc memory[0x%lx 0x%lx]!\n", virt, phys);
		return -EINVAL;
	}
