...
int login_main(int argc UNUSED_PARAM, char **argv)
{
	...
	//printf("wucaiyuan: loginutils/login.c >> login_main\n");
	//printf("wucaiyuan: loginutils/login.c >> login_main\n");
	...
	//printf("wucaiyuan: loginutils/login.c >> run_by_root %d\n", run_by_root);
	//printf("wucaiyuan: loginutils/login.c >> login_main\n");
	//printf("wucaiyuan: loginutils/login.c >> login_main\n");
	//printf("wucaiyuan: loginutils/login.c >> login_main\n");
	...
	username[0] = '\0';
	//printf("##wucaiyuan: argc %d, opt_user %s, argv[0] %s, argv[1] %s\n",
	//		argc, opt_user, argv[0], argv[1]);
	opt = getopt32(argv, "f:h:p", &opt_user, &opt_host);
    //printf("wucaiyuan: main_login >> opt 0x%x\n", opt);
	if (opt & LOGIN_OPT_f) {
		if (!run_by_root)
			bb_error_msg_and_die("-f is for root only");
		safe_strncpy(username, opt_user, sizeof(username));
		//printf("wucaiyuan: for root user[%s]!\n", username);
	}
	argv += optind;
	...
	while (1) {
		//printf("wucaiyuan: main_log in while 1\n");
		/* flush away any type-ahead (as getty does) */
		tcflush(0, TCIFLUSH);

		//printf("wucaiyuan: main_log in while 2\n");
		if (!username[0])
			get_username_or_die(username, sizeof(username));

#if ENABLE_PAM
		//printf("wucaiyuan: main_log in while 3\n");
		pamret = pam_start("login", username, &conv, &pamh);
		if (pamret != PAM_SUCCESS) {
			failed_msg = "start";
			goto pam_auth_failed;
		}
		//printf("wucaiyuan: main_log in while 4\n");
		...
		/* set RHOST */
		//printf("wucaiyuan: main_log in while 5\n");
		if (opt_host) {
		...
		//printf("wucaiyuan: main_log in while 6\n");
		if (!(opt & LOGIN_OPT_f)) {
		...
		//printf("wucaiyuan: main_log in while 7\n");
		...
		//printf("wucaiyuan: main_log in while 8\n");
		/* read user back */
		pamuser = NULL;
		...
		//printf("wucaiyuan: main_log in while 9\n");
		...
		//printf("wucaiyuan: return success!!!!!");
 ...
 pam_auth_failed:
...
#else /* not PAM */
		//printf("wucaiyuan: main_log in while 10\n");
		...
		//printf("wucaiyuan: main_log in while 11\n");
 ...
 fake_it:
		 ...
		 */
		//printf("wucaiyuan: main_log in while 12\n");
