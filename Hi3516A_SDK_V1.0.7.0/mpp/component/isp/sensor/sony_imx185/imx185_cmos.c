...
#define VMAX_1080P60 (1125+INCREASE_LINES)
...
static HI_S32 cmos_get_ae_default(AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    switch(genSensorMode)
    {
        ...
        case WDR_MODE_NONE:   /*linear mode*/
            ...
            pstAeSnsDft->u32InitExposure = 345100;
        break;  
...
static HI_VOID cmos_fps_set(HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if ((WDR_MODE_NONE == genSensorMode)||(WDR_MODE_2To1_FRAME == genSensorMode) || (WDR_MODE_2To1_FRAME_FULL_RATE == genSensorMode))
    {
...
HI_U32 cmos_get_sns_regs_info(ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if (HI_FALSE == bInit)
    {
        ...
        g_stSnsRegsInfo.astSspData[0].u32RegAddr = (SHS1_ADDR & 0xFF);//shutter SHS1[7:0]
        g_stSnsRegsInfo.astSspData[1].u8DelayFrmNum = 0;
        g_stSnsRegsInfo.astSspData[1].u32RegAddr = (SHS1_ADDR & 0xFF) + 1;//shutter SHS1[15:8]
        g_stSnsRegsInfo.astSspData[2].u8DelayFrmNum = 1;//make shutter and gain effective at the same time
        g_stSnsRegsInfo.astSspData[2].u32RegAddr = (GAIN_ADDR & 0xFF);//gain     
        g_stSnsRegsInfo.astSspData[3].u8DelayFrmNum = 0;
        g_stSnsRegsInfo.astSspData[3].u32RegAddr = (VMAX_ADDR & 0xFF);
        g_stSnsRegsInfo.astSspData[4].u8DelayFrmNum = 0;
        g_stSnsRegsInfo.astSspData[4].u32RegAddr = (VMAX_ADDR & 0xFF) + 1;
      
        if (WDR_MODE_BUILT_IN == genSensorMode)
        {
            g_stSnsRegsInfo.astSspData[3].u8DelayFrmNum = 0;
            g_stSnsRegsInfo.astSspData[3].u32RegAddr = (SHS2_ADDR & 0xFF);
            g_stSnsRegsInfo.astSspData[4].u8DelayFrmNum = 0;
            g_stSnsRegsInfo.astSspData[4].u32RegAddr = (SHS2_ADDR & 0xFF) + 1;
            g_stSnsRegsInfo.astSspData[5].u8DelayFrmNum = 0;
            g_stSnsRegsInfo.astSspData[5].u32RegAddr = (VMAX_ADDR & 0xFF);
            g_stSnsRegsInfo.astSspData[6].u8DelayFrmNum = 0;
            g_stSnsRegsInfo.astSspData[6].u32RegAddr = (VMAX_ADDR & 0xFF) + 1;    
            g_stSnsRegsInfo.astSspData[7].u8DelayFrmNum = 0;
            g_stSnsRegsInfo.astSspData[7].u32RegAddr = (HMAX_ADDR & 0xFF);
            g_stSnsRegsInfo.astSspData[8].u8DelayFrmNum = 0;
            g_stSnsRegsInfo.astSspData[8].u32RegAddr = (HMAX_ADDR & 0xFF) + 1;   
        }
        else if((WDR_MODE_2To1_FRAME == genSensorMode) || (WDR_MODE_2To1_FRAME_FULL_RATE == genSensorMode))
        {
            g_stSnsRegsInfo.astSspData[5].u8DelayFrmNum = 1;                                                   
            g_stSnsRegsInfo.astSspData[5].u32RegAddr = (SHS1_ADDR & 0xFF);//shutter SHS1[7:0]                                              
            g_stSnsRegsInfo.astSspData[6].u8DelayFrmNum = 1;                                                   
            g_stSnsRegsInfo.astSspData[6].u32RegAddr = (SHS1_ADDR & 0xFF) + 1;//shutter SHS1[15:8]                                          
        }
